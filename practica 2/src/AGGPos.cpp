...
void selection(population &new_pop, const population &old_pop){
    ...
    // if(!keep_best){
    //     new_pop.solutions[new_pop.solutions.size()-1]=old_pop.solutions[old_pop.best_sol];
    //     new_pop.best_sol=new_pop.solutions.size()-1;
    // }
...
void mutateSolution(solution &sol, const vector<vector<double> > &matrix) {
    ...
    // double old_contrib = contribution(sol.elements, pos1) - matrix[pos1][pos2];
    // double new_contrib = contribution(sol.elements, pos2);
    // sol.fitness = sol.fitness + new_contrib - old_contrib;
}

//void mutation(population &pop,const double &mut_prob, int &evaluations) {
void mutation(population &pop,const double &mut_prob,const vector<vector<double> > &matrix){
    ...
    // if (pop.best_fitness < pop.solutions[pos].fitness) {
    //     pop.best_fitness = pop.solutions[pos].fitness;
    //     pop.best_sol = pos;
    //}
}
...
void cross(population &pop, const double &cross_prob, const int &seed){
    ...
    for(unsigned i = 0; i < num_cross ; i++){
        positionalCross(pop.solutions[2*i], pop.solutions[2*i+1], sol1,seed);
...
void PositionalAGG(const vector<vector<double> > &matrix, const unsigned int &num_sel, const int &seed){
    ...
    cout << setprecision(2) << fixed << best_sol.fitness  << ", " << setprecision(6)<< (double) total/CLOCKS_PER_SEC 
