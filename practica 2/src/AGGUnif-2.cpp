...
no serlo, elimina los elementos sobrantes que contribuyen menos a la solución 
...
void repair(solution &sol, const int &num_sel, const vector<vector<double> > &matrix){
    int selected=0;
    int max_contrib=0, max_pos=-1; 
    double contrib;
    double min_contrib= std::numeric_limits<double>::infinity(), min_pos=-1;
    ...
    while(selected>num_sel){
        ...
        sol.elements[min_pos]=false;
        selected--;
        min_contrib=std::numeric_limits<double>::infinity();
        min_pos=-1;
    }
