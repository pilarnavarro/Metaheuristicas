...
*/
solucion localSearch(const vector<vector<double> > &matriz, const unsigned int &num_sel, const int &seed){
    solucion sol;
    bool mejora;
    ...
    while(mejora and num_eval < 400){  //Iteramos mientras la solución mejore y no se haya superado el número máximo de evaluaciones
        ...
        for(k = 0 ; k < valid_elements.size() and mejora == false and num_eval<400 ; k++){
