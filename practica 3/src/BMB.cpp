...
void localSearch(solucion &sol, const vector<vector<double> > &matriz, const int &seed){
    ...
    mejora=true;
    while(mejora and num_eval < 10000){  //Iteramos mientras la solución mejore y no se haya superado el número máximo de evaluaciones
        mejora = false;
        ...
        for(k = 0 ; k < valid_elements.size() and mejora == false and num_eval < 10000; k++){
...
void BMB(const vector<vector<double> > &matriz, const unsigned int &num_sel, const int &seed){
    clock_t start, total;
    ...
    for(unsigned i=0;i<10;i++){
        randomSolution(sol,num_sel,matriz);
